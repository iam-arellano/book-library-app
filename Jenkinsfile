pipeline {
    ...
    environment {
        ...
        APP_NAME = "book-library"               // Name of your app 
        RELEASE = "1.0.0"
    ...
    stages {
        ...
        stage("Checkout from SCM"){
            steps {
                git branch: 'main', credentialsId: 'github', url: 'https://github.com/iam-arellano/book-library-app.git'
       ...
       //  to scan docker image 
        stage("Trivy Scan") {
           steps {
               script {
	            sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image raemondarellano/book-library:latest --no-progress --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
               }
           }
       }
